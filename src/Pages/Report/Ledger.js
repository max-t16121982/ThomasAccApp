...
const Ledger = () => {
  ...
  const shoot = async () => {
    ...
    if ((CompName != undefined) && (AccName != undefined) && (startDate != undefined) && (endDate != undefined)) {
      ...
      sasdata.push({ refacname: "Dr. Total", balance: 0, cramount: 0, dramount: bdlTotDr })
      sasdata.push({ refacname: "Cr. Total", balance: 0, cramount: bdlTotCr, dramount: 0 })
      sasdata.push({ refacname: "Total", balance: bdlTotBal, cramount: bdlTotCr, dramount: bdlTotDr })
      search = sasdata
      setSearch(search)
      //alert("Sucessfull...")
    }
  ...
  //Whats Connection -- Start
  const formatTableDataForWhatsApp = (data) => {
    //ID\tName\tAge\n
    let message = 'Date\tBill No\tCheque No\tRefrance Name\tAmount\tBalance\n'; // Header row
    data.forEach(row => {
      message += `${row.acdate}\t${row.bill}\t${row.refacname == 'Total' ? Number(row.dramount - row.cramount).toFixed(2) : (row.dramount == 0 ? Number(row.cramount).toFixed(2) : Number(row.dramount).toFixed(2))}\t${Number(row.balance).toFixed(2)}\n`;
    });

    return message;
  };
  //const formattedDataForWhatsApp = formatTableDataForWhatsApp(search);
  ...
  return (
    <div className="App" style={{ marginLeft: '10px', marginRight: '10px' }} >
      ...
      {/* <ReactWhatsapp number="+919978236005" message="Hello Jayla!!! "/> <img src={"./whatsapp.ico"} />
      <a href="https://api.whatsapp.com/send?phone=919978236005" message="abc" target='MANNS'>Contact Us on WhatsApp</a> */}
      {/* <button style={{ border: '2px solid ', height: '35px', width: '60px' }} title="Find Record" onClick={shoot} >
        Search Date, Bill No / Cheque No, Refrance Name, Amount,Balance
      </button> */}
...
*/}
            {search.map((d) => (
              <tr key={d}>
                <td style={{ width: "10%", border: '2px solid green' }}>{d.refacname == 'Total' || d.refacname == 'Cr. Total' || d.refacname == 'Dr. Total' ? '' : (new Date(d.acdate).getDate().toString().length == 1 ? "0" + new Date(d.acdate).getDate() : new Date(d.acdate).getDate())}{d.refacname == 'Total' || d.refacname == 'Cr. Total' || d.refacname == 'Dr. Total' ? '' : '/'}{d.refacname == 'Total' || d.refacname == 'Cr. Total' || d.refacname == 'Dr. Total' ? '' : ((new Date(d.acdate).getMonth() + 1).toString().length == 1 ? "0" + (new Date(d.acdate).getMonth() + 1).toString() : (new Date(d.acdate).getMonth() + 1).toString())}{d.refacname == 'Total' || d.refacname == 'Cr. Total' || d.refacname == 'Dr. Total' ? '' : '/'}{d.refacname == 'Total' || d.refacname == 'Cr. Total' || d.refacname == 'Dr. Total' ? '' : new Date(d.acdate).getFullYear().toString().substring(2)}</td>
                <td style={{ textAlign: 'center', width: "20%", border: '2px solid green' }}>{d.bill} {d.refacname == 'Total' || d.chequeno == "" ? '' : ''}  </td>
                {/* <td>{d.chequeno} + ' No.:' + {d.chequedate}</td> */}
                {/* <td></td> */}
                <td style={d.refacname == 'Total' || d.refacname == 'Cr. Total' || d.refacname == 'Dr. Total' ? { fontWeight: 'bold', width: "30%", border: '2px solid green' } : { width: "30%", border: '2px solid green' }}>{d.refacname}</td>
                <td style={d.cramount == 0 ? { color: 'red', textAlign: 'right', width: "20%", border: '2px solid green' } : (d.refacname == 'Total' ? { fontWeight: 'bold', textAlign: 'right', width: "20%", border: '2px solid green' } : { textAlign: 'right', width: "20%", border: '2px solid green' })} >{d.refacname == 'Total' ? Number(d.dramount - d.cramount).toFixed(2) : (d.dramount == 0 ? Number(d.cramount).toFixed(2) : Number(d.dramount).toFixed(2))}</td>
                <td style={d.refacname == 'Total' ? { fontWeight: 'bold', textAlign: 'right', width: "20%", border: '2px solid green' } : { textAlign: 'right', width: "20%", border: '2px solid green' }}>{Number(d.balance).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
